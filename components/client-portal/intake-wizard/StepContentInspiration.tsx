...
interface StepContentInspirationProps {
  entries: InspirationEntry[];
  onChange: (entries: InspirationEntry[]) => void;
}

const LINKEDIN_PERSON_RE = /^https?:\/\/(www\.)?linkedin\.com\/in\//i;

function validateLinkedInPersonUrl(url: string): string | null {
  if (!url.trim()) return null;
  if (!LINKEDIN_PERSON_RE.test(url.trim())) {
    return 'Must be a LinkedIn personal profile URL (linkedin.com/in/...)';
  }
  return null;
}

const StepContentInspiration: React.FC<StepContentInspirationProps> = ({ entries, onChange }) => {
  const [expandedNotes, setExpandedNotes] = useState<Set<number>>(new Set());
  const [touchedUrls, setTouchedUrls] = useState<Set<number>>(new Set());

  const updateEntry = (index: number, field: keyof InspirationEntry, value: string) => {
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold text-gray-900 dark:text-zinc-100 mb-2">
        Whose content do you admire on LinkedIn?
      </h2>
      <p className="text-sm text-gray-500 dark:text-zinc-400 mb-6">
        Paste LinkedIn profiles of 2-5 creators whose content style, tone, or approach you look up
        to. We'll analyze their style to shape your content strategy.
      </p>

      <div className="space-y-4">
        {entries.map((entry, index) => {
          const urlError = touchedUrls.has(index) ? validateLinkedInPersonUrl(entry.url) : null;
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <input
                    ...
                    placeholder="https://linkedin.com/in/creator-name"
              ...
              <div className="ml-8 mt-2">
                ...
                {expandedNotes.has(index) && (
                  <textarea
                    ...
                    rows={2}
                    placeholder="e.g. Love their storytelling style, direct and punchy, great at hooks..."
...
export default StepContentInspiration;
